...
| **Streaming accumulation** | ✅ parity on all six modelled delta kinds — with 5 recorded divergences (§12 rows 3, 18-21) and 1 gap owned elsewhere: `input_json_delta` gating still checks `ContentBlock::ToolUse` only, so `ServerToolUse` arguments still stream unbuffered even though the block is now typed (§4.2) |
...
| # | Item | Class | Why here |
|---|---|---|---|
| 1 | Streaming accumulator: buffer `input_json_delta`, concat `thinking_delta`, replace on `signature_delta` (§4.2) | ✅ **delivered** | Was: silent wrong data, streaming tool use unusable with no signal of failure. Now handled in `MessageAccumulator`; end state matches Python/TS per-rule (§4.2 delivered-state table). Three rows in §4.2's delivered-state table are not ✅: one untracked gap — `input_json_delta` gating still checks `ContentBlock::ToolUse` only, so `ServerToolUse` arguments still stream unbuffered even though the block is now typed (§4.2) — and two recorded divergences, rows 18 and 19. |
