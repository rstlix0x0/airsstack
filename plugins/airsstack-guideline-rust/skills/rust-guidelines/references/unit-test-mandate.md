...
Reinforces `M-STATIC-VERIFICATION` and the test discipline in the strict-quality reference. Stricter than the Microsoft guideline: this rule defines *where* tests must live, not just *that* they exist.
...
1. **Export-only modules.** `lib.rs`, `mod.rs`, and any file that contains exclusively `//!` module docs + `mod` / `pub mod` / `pub use` statements. Enforced by the mod-rs-export-only reference. Examples: `src/lib.rs`, `src/types/mod.rs`, `src/transport/mod.rs`.
...
3. **Pure trait definitions.** A file whose only item is a `pub trait Foo { … }` declaration (no default-method bodies with branching logic, no `impl` blocks for foreign types). Example: `storage/seam.rs` (`Storage` trait).
...
This makes the exemption visible to reviewers without requiring grep against any external rules file.
...
Concretely: a file like `storage/sql_impl.rs` containing a `classify_storage_error` function MUST have an inline `#[cfg(test)] mod tests` for `classify_storage_error` — even if `tests/storage_sql.rs` end-to-end covers the happy path through `save()`. Integration coverage of the call site does not substitute for unit coverage of the classifier.
...
- **Validation invariants** for parse-don't-validate ctors (per the strong-types reference) — both accept and reject sides.
...
- **Panic-on-bypass** paths where a struct literal can violate an invariant the constructor guards.
...
- A test uses `#[ignore]` without an inline reason + tracking link (already covered by the strict-quality reference; called out here because ignored tests in the unit block are a common loophole).

## Rationale

Three concrete failure modes this rule prevents:

1. **Hidden classifier logic.** `classify_storage_error` in `storage/sql_impl.rs` shipped without inline tests because the integration test exercised one path through it. The other classification branches went unverified at the unit level — a regression in the classification heuristic would only surface as a misclassified error category in production.
2. **Silent invariant erosion.** `credentials.rs` ships a `Debug` impl whose entire purpose is masking a secret. Without an inline `Debug` test, a future derive-macro change or refactor could quietly start leaking the token, and no integration test would catch it because integration tests don't typically format `SecretToken`.
3. **Accessor drift.** Accessor methods are trivial individually, but they form the contract that downstream sub-resources rely on. Inline tests pin the contract to the file that owns it.
...
- The mod-rs-export-only reference — supplies exemption #1; `mod.rs` / `lib.rs` carry no logic to test.
- The strict-quality reference — covers the DoD command set including `cargo test`. This rule adds the *placement* requirement.
- The strong-types reference — newtype validating constructors are exactly the "parse-don't-validate" tests this rule mandates.
- The doc-comment-discipline reference — doctests on public items remain required by `M-DESIGN-FOR-AI`; doctests and unit tests are complementary, not substitutes for each other.
